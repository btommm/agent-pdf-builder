...
| Token | Hex | Role |
|---|---|---|
| navy | `#0B1F33` | Header/footer bars, table header, H1/H2 |
| navy-2 | `#13293D` | Body text |
| steel | `#4A5560` | Captions, blanks, page furniture |
| silver | `#C5C9CE` | Footer secondary text |
| red | `#8B1E1E` | 3pt accent rule under header; table-header underline |
...
Implement chrome in `onFirstPage` / `onLaterPages`. See `scripts/dossier_theme.py`.
...
Drop unused sections. Do not leave a numbered hole. Renumber.
...
2. Call `build_doc(path, title, author)` for the `SimpleDocTemplate`.
3. Use `styles()`, `header_footer_fn(owner, kind, year, classification, constraint)`, `section_rule()`, `make_table()`, `blank_field()`, `checkbox()`.
4. Write to `/home/workdir/artifacts/<slug>_<YYYY-MM-DD>.pdf`.
5. Run `pdfinfo` and spot-check page 1 chrome. Fix overflow before handing the file over.
